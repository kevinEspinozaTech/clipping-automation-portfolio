...
A direct WebFetch of every comparable channel's YouTube "About" page was attempted first, per the audit's request. Every single attempt was redirected to YouTube's cookie-consent wall (`consent.youtube.com`); following the consent redirect through to completion returned only page-footer navigation links, with no channel statistics rendered — YouTube's channel header is client-side-rendered and not present in the fetched HTML in this environment. This is a genuine environment limitation, not a skipped step. As a fallback, Social Blade (a standard, widely-used third-party YouTube-statistics tracker) was attempted and returned HTTP 403 (bot-blocked) for every channel; **HypeAuditor was also tried and likewise returned HTTP 403 on every direct-fetch attempt (confirmed across two separate review passes)**, so any HypeAuditor-sourced figure in this project is a secondary, unverified data point, not an OBSERVED fact — see the 2026-09-21 cleanup note below. The best available real verification was therefore dated, cited figures surfaced via web search (Wikipedia and similar aggregators/encyclopedic sources that could themselves be fetched and read directly) — weaker than a live YouTube fetch, but stronger than the single-blog citations used in the first pass, and every figure now carries a specific date.

**Cleanup (2026-09-21):** every HypeAuditor-sourced figure in this project's CSVs (RealLifeLore's demand score, Memorias de Pez's audience estimate) has been reclassified from `OBSERVED` to `INFERRED`/`NOT_PUBLICLY_AVAILABLE`, since the site itself cannot be independently verified by fetch. No score changed as a result — in both cases, the underlying conclusion (RealLifeLore is a large, established channel; Memorias de Pez has a real, substantial Spanish-language audience) was already independently supported by a separate source (the original Overseeros blog range for RealLifeLore; a second, independent order-of-magnitude estimate for Memorias de Pez), so the HypeAuditor figures were corroborating detail, not the sole basis for either score. Full detail: [`../templates/niche-scoring-matrix.csv`](../templates/niche-scoring-matrix.csv) rows NLOG-0001 and NLOG-0009, and [`../templates/channel-video-registry.csv`](../templates/channel-video-registry.csv) rows CH-01 and CH-17.

### Metrics newly confirmed or corrected

- RealLifeLore: 7.9-9M subscribers (Overseeros blog range, the actual basis for this score) — a more precise HypeAuditor figure (~7.94M) was also seen but is `NOT_PUBLICLY_AVAILABLE` for independent verification; see the 2026-09-21 cleanup note above.
...
|---|---|---|---|
| Geography | Daniel Geohistoria (~235K subs) | Memorias de Pez (adjacent history/geopolitics/curiosities content; two unverified secondary estimates, ~2.7M and ~2.69M, roughly agree on an order of magnitude of low millions, but neither could be independently confirmed — see the 2026-09-21 HypeAuditor cleanup) | No channel matching the exact geography/borders angle at large scale, and no independently-verified subscriber count for the closest adjacent one |
